...
<head>
<title>{{ config('app.name', 'PUC Medic Doctor Dashboard') }}</title>
<meta charset="utf-8">
...
<style>
    ...
    /* Utility class for Bootstrap 5 badges since the user's template uses Bootstrap 4 links */
    .badge.bg-success { background-color: var(--accent-color) !important; color: white !important; }
    .badge.bg-warning { background-color: orange !important; color: black !important; }
...
<span class="navbar-toggler-icon"></span>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <ul class="navbar-nav ms-auto">
                @guest
                    {{-- Authentication links here if needed --}}
                @else
...
<nav id="sidebarMenu">
    <div class="sidebar-sticky pt-3">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item">
                {{-- <a class="nav-link {{ Request::routeIs('doctor.patients.index') ? 'active' : '' }}"
                   href="{{ route('doctor.patients.index') }}"> --}}
                    <i class="fa-solid fa-bed"></i> My Patients
                </a>
            </li>
            <li class="nav-item">
                {{-- <a class="nav-link {{ Request::routeIs('doctor.records.index') ? 'active' : '' }}"
                   href="{{ route('doctor.records.index') }}"> --}}
                    <i class="fa-solid fa-clipboard-list"></i> Medical Records
                </a>
            </li>
            <li class="nav-item">
                {{-- <a class="nav-link {{ Request::routeIs('doctor.prescriptions') ? 'active' : '' }}"
                   href="{{ route('doctor.prescriptions') }}"> --}}
